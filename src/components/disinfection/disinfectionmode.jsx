...
class Disinfectionmode extends Component {
    ...
    componentDidMount(){
        this.state.waypoint_sub.subscribe(function(message) {
        ...
        this.state.motor_sub.subscribe(function(message) {
    ...
    render() { 
        return ( 
            <React.Fragment>
                ...
                <span className={this.textColour()}>Current Waypoint: {this.state.status_waypoint}</span>
                <button onClick = {()=>this.handleStart()} className="btn btn-primary btn-sm m-2" >Start Disinfection Mode</button>
                <button onClick = {()=>this.handleStop()} className="btn btn-danger btn-sm m-2" >Stop Disinfection Mode</button>
            </React.Fragment>
        );
    }
